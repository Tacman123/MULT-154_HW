...
public class NavPlayerMovement : MonoBehaviour
{
    ...
    {
        Vector3 targetPosittion = transform.position + transform.forward * 6;
        while(Vector3.Distance (lookTarget.transform.position, targetPosittion) > 0.01f)
        {
            targetPosittion = transform.position + transform.forward * 6;
            lookTarget.transform.position = Vector3.Lerp(lookTarget.transform.position, targetPosittion, 0.08f);
            yield return new WaitForSeconds(0.05f);
        }
        lookTarget.transform.position = targetPosittion;
    }
}
